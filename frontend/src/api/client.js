...
export function getApiErrorMessage(error, fallback = 'Ocurrió un error. Intenta nuevamente.') {
  ...
  if (error.response?.status === 401) return 'Tu sesión no es válida o ha expirado. Inicia sesión nuevamente.';
  ...
  if (error.response?.status >= 500) return 'El servidor encontró un problema. Revisa la consola del backend para más detalles.';
